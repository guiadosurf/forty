...
function handleSearchQuery(event) {
  ...
  if (!query) {
    displayErrorMessage("Busque por algo!");
    return;
  }
  const results = searchSite(query)
  if (!results.length) {
    displayErrorMessage("Desculpe, não encontramos nada")
    return
  }
...
function updateSearchResults(query, results) {
  ...
  document.getElementById("results-count-text").innerHTML = searchResultListItems.length > 1 ? " resultados" : " resultado";
